  ...
  (function injectFoebeBreadcrumbFast() {
    ...
    function injectCss() {
      ...
      var css = [
        ...
        ".foebe-breadcrumb li:not(:last-child)::after{content:'›';color:#BB7E60;opacity:.68;font-weight:700;}",
        ".foebe-breadcrumb a,.foebe-breadcrumb span{color:currentColor;text-decoration:none;}",
        ".foebe-breadcrumb a{opacity:.92;padding:3px 7px;border-radius:999px;background:rgba(187,126,96,.11);border:1px solid rgba(187,126,96,.20);transition:color .18s ease,background .18s ease,border-color .18s ease,opacity .18s ease;}",
        ".foebe-breadcrumb a:hover,.foebe-breadcrumb a:focus-visible{color:#F0EAE7;background:#BB7E60;border-color:#BB7E60;opacity:1;outline:none;}",
        ".foebe-breadcrumb li > span{opacity:.70;}",
        ".foebe-breadcrumb [aria-current='page']{color:#BB7E60!important;font-weight:700;opacity:1;}",
        "[data-theme='day'] .foebe-breadcrumb a{color:#4E291F;background:rgba(78,41,31,.055);border-color:rgba(78,41,31,.14);}",
        "[data-theme='day'] .foebe-breadcrumb a:hover,[data-theme='day'] .foebe-breadcrumb a:focus-visible{color:#F0EAE7;background:#BB7E60;border-color:#BB7E60;}",
        "@media(max-width:520px){.foebe-breadcrumb{width:100%;max-width:100%;padding:8px 10px;border-radius:18px;font-size:11.4px;margin-bottom:18px;}.foebe-breadcrumb ol{gap:5px;}.foebe-breadcrumb li{gap:5px;}.foebe-breadcrumb a{padding:3px 6px;}}",
    ...
    function renderTrail(items) {
      return items.map(function (item) {
        ...
        if (item.current) return '<li><span aria-current="page">' + label + '</span></li>';
        if (item.href) return '<li><a href="' + escapeHtml(item.href) + '" title="Aller à ' + label + '">' + label + '</a></li>';
