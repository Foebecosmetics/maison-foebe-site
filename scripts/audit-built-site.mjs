...
for (const file of pageFiles) {
  ...
  for (const duplicateId of duplicateIds) errors.push(`${rel} : id dupliqué → ${duplicateId}`);
  const skipTag = tags(html, 'a').find((tag) => attr(tag, 'id') === 'foebeSkipLink');
  if (!skipTag) errors.push(`${rel} : lien d’évitement global absent`);
  else {
    const skipHref = attr(skipTag, 'href');
    if (!skipHref?.startsWith('#')) errors.push(`${rel} : cible du lien d’évitement invalide`);
    else {
      const targetId = skipHref.slice(1);
      if (!new RegExp(`\\bid=["']${targetId.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&')}["']`, 'i').test(html)) {
        errors.push(`${rel} : cible du lien d’évitement absente → ${skipHref}`);
      }
    }
  }
  ...
  if (route === '/a-propos/' && (!types.has('ProfilePage') || !types.has('Person') || !types.has('EducationalOccupationalCredential'))) errors.push('À propos : ProfilePage/Person/credential incomplet');
  if (!is404 && route !== '/mentions-legales/') {
    const graphNodes = structured.flatMap((item) => Array.isArray(item?.['@graph']) ? item['@graph'] : [item]);
    const page = graphNodes.find((node) => node?.['@id'] === `${siteOrigin}${route}#webpage`);
    if (!page?.dateModified) errors.push(`${rel} : dateModified absente du nœud de page`);
    if (page?.publishingPrinciples !== `${siteOrigin}/cadre-editorial/`) errors.push(`${rel} : publishingPrinciples absent ou incorrect`);
    if (page?.author?.['@id'] !== `${siteOrigin}/a-propos/#fondatrice`) errors.push(`${rel} : auteur éditorial absent ou incohérent`);
  }
  if (route === '/faq/') {
    ...
    const guidedLinks = (html.match(/<a\b[^>]*class=["'][^"']*faq-item__next/gi) ?? []).length;
    const visibleGroups = (html.match(/<section\b[^>]*class=["'][^"']*faq-group(?:\s|["'])/gi) ?? []).length;
    const jumpLinks = (html.match(/<nav\b[^>]*class=["'][^"']*faq-jump[^"']*["'][^>]*>[\s\S]*?<\/nav>/gi) ?? [])
      .flatMap((block) => block.match(/<a\b/gi) ?? []).length;
    if (visibleQuestions < 25 || visibleQuestions > 30) errors.push(`FAQ : ${visibleQuestions} questions visibles (attendu entre 25 et 30)`);
    if (guidedLinks !== visibleQuestions) errors.push(`FAQ : ${guidedLinks} prochaines étapes pour ${visibleQuestions} réponses (attendu exactement une par réponse)`);
    if (visibleGroups !== 6) errors.push(`FAQ : ${visibleGroups} sections visibles (attendu 6)`);
    if (jumpLinks !== 4) errors.push(`FAQ : ${jumpLinks} portes d’entrée dans le héros (attendu 4)`);
...
for (const [route, minimum] of [['/lexique/', 3], ['/pratiquer/', 3], ['/a-propos/', 2], ['/boussole-scenarios/', 2], ['/faq/', 3], ['/cadre-editorial/', 3]]) {
...
if (!(await exists(redirectsPath))) errors.push('_redirects absent');
  ...
  const legacy = [
    ...
    '/a-propos.html','/faq.html','/cadre-editorial.html','/mentions-legales.html','/zone-corps.html','/zone-energie.html','/zone-emotions.html',
...
if (!homeHtml.includes('href="/cadre-editorial/"') || countOccurrences(shellScript, '/cadre-editorial/') < 1) {
  errors.push('Shell : le lien Cadre éditorial doit être présent dans la navigation rendue et le footer');
}
if (!homeHtml.includes('href="/faq/"') || countOccurrences(shellScript, '/faq/') < 1) {
  errors.push('Shell : le lien FAQ doit être présent dans la navigation rendue et le footer');
}
...
if (!(await exists(shellCssPath))) errors.push('Shell : feuille CSS externe absente');
  ...
  const shellCss = await fs.readFile(shellCssPath, 'utf8');
  if (!shellCss.includes('100dvh')) errors.push('Shell : le panneau mobile doit utiliser 100dvh');
  if (/max-height:calc\(100vh\s*-\s*(?:58|60)px\)/.test(shellCss)) errors.push('Shell : une ancienne hauteur 100vh subsiste sur le panneau');
  if (shellCss.includes('foebe-story-immersive .theme-toggle{display:none')) errors.push('Shell : le bouton jour/nuit ne doit jamais être retiré en mode immersif');
}
if (shellScript.includes('FOEBE CLEANUP UNIFIÉ') || shellScript.includes('fallbackNav')) {
  errors.push('Shell : un ancien nettoyage tardif ou remplacement de navigation subsiste');
}
if (!shellScript.includes('pointerenter') || !shellScript.includes('Menu ouvert')) {
  errors.push('Shell : le survol ergonomique du menu desktop est absent');
}
for (const file of pageFiles) {
  const html = await fs.readFile(file, 'utf8');
  const rel = path.relative(dist, file).replaceAll(path.sep, '/');
  if (countOccurrences(html, 'id="mainNav"') !== 1) errors.push(`${rel} : navigation globale absente ou dupliquée`);
  if (countOccurrences(html, 'id="navMenu"') !== 1) errors.push(`${rel} : panneau global absent ou dupliqué`);
  const expectedThemeButtons = rel === 'lexique/index.html' ? 0 : 1;
  if (countOccurrences(html, 'id="themeToggle"') !== expectedThemeButtons) {
    errors.push(`${rel} : nombre de boutons jour/nuit incorrect (attendu ${expectedThemeButtons})`);
  }
  if (html.includes('id="fallbackNav"')) errors.push(`${rel} : ancienne navigation de secours encore rendue`);
}
for (const route of ['/lexique/', '/dictionnaire/', '/dictionnaire/skincare/']) {
  const html = await fs.readFile(outputFileForPath(route), 'utf8');
  if (!/<html\b[^>]*data-foebe-nav="dark"/i.test(html)) errors.push(`${route} : navigation sombre non déclarée globalement`);
}
for (const file of pageFiles) {
  const html = await fs.readFile(file, 'utf8');
...
// Garde-fous Phase C — continuité des CSS et états de thème.
const requiredBodyClasses = new Map([
  ['/', ['foebe-page', 'foebe-page-index', 'page-index']],
  ['/comprendre/', ['foebe-page', 'foebe-page-comprendre', 'page-comprendre']],
  ['/a-propos/', ['foebe-page', 'foebe-page-a-propos', 'page-a-propos']],
  ['/methode/', ['foebe-page', 'foebe-page-methode', 'page-methode']],
  ['/zones/', ['foebe-page', 'foebe-page-zones', 'page-zones-premium']],
  ['/respiration/', ['foebe-page', 'foebe-page-respiration']],
]);
for (const [route, requiredClasses] of requiredBodyClasses) {
  const html = await fs.readFile(outputFileForPath(route), 'utf8');
  const bodyTag = html.match(/<body\b[^>]*>/i)?.[0] ?? '';
  const bodyClasses = new Set((attr(bodyTag, 'class') ?? '').split(/\s+/).filter(Boolean));
  for (const requiredClass of requiredClasses) {
    if (!bodyClasses.has(requiredClass)) errors.push(`${route} : classe de continuité CSS absente → ${requiredClass}`);
  }
}
const lexiconHtml = await fs.readFile(outputFileForPath('/lexique/'), 'utf8');
if (!/<html\b[^>]*data-foebe-theme-locked="night"/i.test(lexiconHtml)) errors.push('Lexique : verrouillage nuit absent');
for (const route of ['/dictionnaire/', '/dictionnaire/mode-emploi/', '/dictionnaire/skincare/']) {
  const html = await fs.readFile(outputFileForPath(route), 'utf8');
  if (/data-foebe-theme-locked=/i.test(html)) errors.push(`${route} : le Dictionnaire doit garder les modes jour et nuit`);
  if (countOccurrences(html, 'id="themeToggle"') !== 1) errors.push(`${route} : bouton jour/nuit absent ou dupliqué`);
}
for (const cssFile of (await walk(path.join(root, 'src', 'styles'))).filter((file) => file.endsWith('.css'))) {
  const css = await fs.readFile(cssFile, 'utf8');
  for (const legacyMarker of ['fallbackNav', '.fallback-nav', 'foebe-scroll-progress--fallback', 'foebeScrollProgressFallbackBar']) {
    if (css.includes(legacyMarker)) errors.push(`${path.relative(root, cssFile)} : correctif CSS obsolète subsiste → ${legacyMarker}`);
  }
}
for (const sourceFile of sourceFiles) {
  const source = await fs.readFile(sourceFile, 'utf8');
  if (source.includes('foebe-scroll-progress--fallback') || source.includes('foebeScrollProgressFallbackBar')) {
    errors.push(`${path.relative(root, sourceFile)} : fallback de progression obsolète subsiste`);
  }
}
const manifest = JSON.parse(await fs.readFile(path.join(dist, 'site.webmanifest'), 'utf8'));
const expectedGlobalDescription = 'Maison Foébé est un espace structuré pour comprendre ce qui fatigue, ce qui épuise et ce qui empêche de récupérer, puis choisir un geste concret.';
if (manifest.description !== expectedGlobalDescription) errors.push('Manifest : description globale non alignée');
if (!homeHtml.includes(expectedGlobalDescription)) errors.push('Accueil : description globale attendue absente des métadonnées structurées');


// Garde-fous Phase C — règles CSS historiques réellement nécessaires.
const requiredCssRules = [
  ['src/styles/pages/lexique.css', [
    /\.seo-hidden\s*\{/,
    /left\s*:\s*-9999px/,
    /width\s*:\s*1px/,
    /overflow\s*:\s*hidden/,
  ]],
  ['src/styles/pages/dictionnaire.css', [
    /\.mobile-retour-fab\s*\{/,
    /body\.focus-mode\s+\.mobile-retour-fab\s*\{/,
    /\[data-theme=["']night["']\]\s+\.chooser-open\s*,/,
    /\[data-theme=["']night["']\]\s+\.chooser-open:hover\s*,/,
  ]],
  ['src/styles/foebe-zone-shared.css', [
    /\.zone-hero\s*>\s*\*\s*\{/,
    /z-index\s*:\s*1/,
  ]],
];
for (const [relativeCssPath, patterns] of requiredCssRules) {
  const css = await fs.readFile(path.join(root, relativeCssPath), 'utf8');
  for (const pattern of patterns) {
    if (!pattern.test(css)) errors.push(`${relativeCssPath} : règle CSS de continuité absente → ${pattern}`);
  }
}
...
// Garde-fous Phase C — autorité vérifiable, transparence et hébergement exact.
const aboutHtml = await fs.readFile(path.join(dist, 'a-propos', 'index.html'), 'utf8');
for (const requiredReference of [
  'https://theses.hal.science/tel-02860264',
  'https://theses.fr/2019AZUR2027',
  '2021inpact098.pdf',
  'livre_des_r%C3%A9sum%C3%A9s_aiptlf_bordeaux_2018.pdf',
]) {
  if (!aboutHtml.includes(requiredReference)) errors.push(`À propos : référence vérifiable absente → ${requiredReference}`);
}
for (const staleClaim of ['Revue PTO', 'chapitre d’ouvrage L’Harmattan', 'revue AFNOR', 'Références complètes et liens à venir']) {
  if (aboutHtml.includes(staleClaim)) errors.push(`À propos : affirmation non documentée encore publiée → ${staleClaim}`);
}
if (!aboutHtml.includes('Une présence sobre, par choix')) errors.push('À propos : engagement de présence sobre absent');
const editorialHtml = await fs.readFile(path.join(dist, 'cadre-editorial', 'index.html'), 'utf8');
for (const marker of ['Hiérarchie des sources', 'Vérification et mise à jour', 'Indépendance éditoriale', 'Soutenir l’autonomie, pas la dépendance', 'contact@maisonfoebe.fr']) {
  if (!editorialHtml.includes(marker)) errors.push(`Cadre éditorial : engagement absent → ${marker}`);
}
const legalHtml = await fs.readFile(path.join(dist, 'mentions-legales', 'index.html'), 'utf8');
if (legalHtml.includes('Netlify')) errors.push('Mentions légales : ancien hébergeur Netlify encore présent');
if (!legalHtml.includes('Cloudflare, Inc.')) errors.push('Mentions légales : hébergeur Cloudflare absent');

// Garde-fou D1 — les liens cross-page vers le formulaire de l'accueil doivent toujours aboutir.
const faqHtmlForAnchors = await fs.readFile(path.join(dist, 'faq', 'index.html'), 'utf8');
const portesLinks = countOccurrences(faqHtmlForAnchors, 'href="/#portes"');
if (portesLinks < 1) errors.push('FAQ : aucun lien vers /#portes');
if (!/\bid=["']portes["']/.test(homeHtml)) errors.push('Accueil : ancre #portes absente');
const portesSection = homeHtml.match(/<section\b[^>]*\bid=["']portes["'][^>]*>[\s\S]*?<\/section>/i)?.[0] ?? '';
if (!portesSection.includes('id="newsletterForm"')) errors.push('Accueil : le formulaire email ne se trouve pas dans la section #portes');
...
console.log(`\n✅ Audit Migration Astro — Phases A + B + C réussi : ${pageFiles.length} pages HTML, ${indexableRoutes.size} pages indexables, zéro incohérence bloquante.`);
