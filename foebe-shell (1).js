...
(function () {
  ...
  var SHELL_CSS = [
    ...
    /* ── Fil d'Ariane ── */
    "#foebeBreadcrumb{position:relative;z-index:100;width:100%;background:transparent;padding:72px 0 0;}",
    "#foebeBreadcrumb nav{max-width:1040px;margin:0 auto;padding:10px clamp(16px,3vw,28px);display:flex;align-items:center;flex-wrap:wrap;gap:4px 6px;}",
    "#foebeBreadcrumb ol{list-style:none;display:flex;flex-wrap:wrap;align-items:center;gap:4px 6px;margin:0;padding:0;}",
    "#foebeBreadcrumb li{display:flex;align-items:center;gap:6px;font-family:'Poppins',sans-serif;font-size:11.5px;font-weight:500;line-height:1;}",
    "#foebeBreadcrumb li+li::before{content:'›';color:var(--warm,#BB7E60);opacity:.7;font-size:12px;line-height:1;}",
    "#foebeBreadcrumb a{color:var(--warm,#BB7E60);text-decoration:none;opacity:.78;transition:opacity .18s ease;}",
    "#foebeBreadcrumb a:hover{opacity:1;text-decoration:underline;}",
    "#foebeBreadcrumb [aria-current='page']{color:var(--text,#F0EAE7);opacity:.92;font-weight:600;}",
    "[data-theme='day'] #foebeBreadcrumb [aria-current='page']{color:#4E291F;}",
    "[data-theme='day'] #foebeBreadcrumb a{color:#BB7E60;}",
    "@media(max-width:640px){#foebeBreadcrumb{padding-top:66px;}#foebeBreadcrumb nav{padding:8px clamp(14px,4vw,20px);}#foebeBreadcrumb li{font-size:11px;}}"
  ].join("\n");
  ...
  /* ═══════════════════════════════════════════════════════════════════════════
     2.5 FIL D'ARIANE — carte de navigation contextuelle
  ═══════════════════════════════════════════════════════════════════════════ */
  var BREADCRUMB_MAP = {
    "index.html":                   null, // pas de fil sur l'accueil
  ...
  function injectBreadcrumb() {
    ...
    if (!crumbs || crumbs.length === 0) return; // accueil : pas de fil

    var ol = crumbs.map(function(c, i) {
      var isLast = i === crumbs.length - 1;
      if (isLast) {
        return '<li><span aria-current="page">' + c.label + '</span></li>';
      }
      return '<li><a href="' + c.href + '">' + c.label + '</a></li>';
    }).join('');

    var bc = document.createElement('div');
    bc.id = 'foebeBreadcrumb';
    bc.innerHTML = '<nav aria-label="Fil d'ariane"><ol>' + ol + '</ol></nav>';

    // Insérer juste après la nav principale
